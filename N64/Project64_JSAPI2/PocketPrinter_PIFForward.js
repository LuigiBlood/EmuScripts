...
if (typeof PJ64_JSAPI_VERSION !== 'undefined') {
	...
	*/
	...
	function SaveColorImage()
	{
		...
		for (; fs.exists(filename + num + ".png"); num++);
		filename += num;
	...
	function ProcessPort(sent, recv)
	{
		...
		{
			//Info / Reset
			if (sent.length != 0x01 || recv.length != 0x03)
			{
		...
		{
			//MBC4 Read
			if (sent.length != 0x03 || recv.length != 0x21)
			{
				//Return Error if the send/recv info is wrong
				return -1;
			}
			...
			const contaddr = sent.readUInt16BE(1) & 0xFFE0;
			const contcrca = sent.readUInt16BE(1) & 0x001F;

			//Receive Data with CRC
			cont_ram.copy(recv, 0, contaddr, contaddr + 0x20);

			var contdata = cont_ram.slice(contaddr, contaddr + 0x20);
			recv[0x20] = PIFForward.crc8(contdata);
		...
		{
			//MBC4 Write
			if (sent.length != 0x23 || recv.length != 0x01)
			{
				//Return Error if the send/recv info is wrong
				return -1;
			}
			...
			//Copy Data
			sent.copy(cont_ram, contaddr, 3, 0x23);

			var contdata = cont_ram.slice(contaddr, contaddr + 0x20);
			recv[0] = PIFForward.crc8(contdata);
